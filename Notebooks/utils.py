...
def get_data(data_path='../Data/tom_sample_data.zip', target_file='tom_commits_metrics.csv', target_metrics=['total_changed']):
    cols_to_select = ['commit_datetime', 'full_name'] + target_metrics
    ...
    for name, group in repository_hist_df.groupby('full_name'):
        ...
        if a.days < 365//2 or group.shape[0] < 50: continue
        ...
        for target_metric in target_metrics:
            result[name][target_metric] = group[target_metric].values.astype(np.float64)
